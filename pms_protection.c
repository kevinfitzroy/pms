...
void Prot_UpdateInputs(float soc, float cell_voltage)
{
    ...
    g_filt_volt = g_config.alpha * cell_voltage + (1.0f - g_config.alpha) * g_filt_volt;
}
...
ProtAction_t Prot_GetSuggestion(void)
{
    ...
    for (size_t i = 0; i < g_config.num_rules; ++i) {
        ...
        if (g_filt_soc <= r->soc_threshold || g_filt_volt <= r->volt_threshold) {
            return r->action;
        }
    }
